# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'export_cleaned_lines.ui'
#
# Created by: PyQt4 UI code generator 4.11.4
#
# WARNING! All changes made in this file will be lost!

from PyQt4 import QtCore, QtGui

try:
    _fromUtf8 = QtCore.QString.fromUtf8
except AttributeError:
    def _fromUtf8(s):
        return s

try:
    _encoding = QtGui.QApplication.UnicodeUTF8
    def _translate(context, text, disambig):
        return QtGui.QApplication.translate(context, text, disambig, _encoding)
except AttributeError:
    def _translate(context, text, disambig):
        return QtGui.QApplication.translate(context, text, disambig)

class Ui_Dialog(object):
    def setupUi(self, Dialog):
        Dialog.setObjectName(_fromUtf8("Dialog"))
        Dialog.resize(529, 613)
        Dialog.setStyleSheet(_fromUtf8("background-color: rgb(48, 48, 48);\n"
"gridline-color: rgb(195, 195, 195);\n"
"color: rgb(255, 255, 255);\n"
""))
        self.gridLayout_4 = QtGui.QGridLayout(Dialog)
        self.gridLayout_4.setObjectName(_fromUtf8("gridLayout_4"))
        self.frame_6 = QtGui.QFrame(Dialog)
        sizePolicy = QtGui.QSizePolicy(QtGui.QSizePolicy.Preferred, QtGui.QSizePolicy.Preferred)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.frame_6.sizePolicy().hasHeightForWidth())
        self.frame_6.setSizePolicy(sizePolicy)
        self.frame_6.setMinimumSize(QtCore.QSize(0, 58))
        self.frame_6.setFrameShape(QtGui.QFrame.StyledPanel)
        self.frame_6.setFrameShadow(QtGui.QFrame.Raised)
        self.frame_6.setObjectName(_fromUtf8("frame_6"))
        self.label_3 = QtGui.QLabel(self.frame_6)
        self.label_3.setGeometry(QtCore.QRect(10, 10, 821, 20))
        font = QtGui.QFont()
        font.setBold(True)
        font.setWeight(75)
        self.label_3.setFont(font)
        self.label_3.setStyleSheet(_fromUtf8("color: rgb(163, 163, 163);"))
        self.label_3.setFrameShape(QtGui.QFrame.NoFrame)
        self.label_3.setFrameShadow(QtGui.QFrame.Raised)
        self.label_3.setObjectName(_fromUtf8("label_3"))
        self.label_4 = QtGui.QLabel(self.frame_6)
        self.label_4.setGeometry(QtCore.QRect(10, 30, 441, 21))
        self.label_4.setStyleSheet(_fromUtf8("color: rgb(163, 163, 163);"))
        self.label_4.setObjectName(_fromUtf8("label_4"))
        self.gridLayout_4.addWidget(self.frame_6, 0, 0, 1, 1)
        self.frame_2 = QtGui.QFrame(Dialog)
        self.frame_2.setFrameShape(QtGui.QFrame.StyledPanel)
        self.frame_2.setFrameShadow(QtGui.QFrame.Raised)
        self.frame_2.setObjectName(_fromUtf8("frame_2"))
        self.gridLayout_3 = QtGui.QGridLayout(self.frame_2)
        self.gridLayout_3.setObjectName(_fromUtf8("gridLayout_3"))
        self.label_2 = QtGui.QLabel(self.frame_2)
        self.label_2.setFrameShape(QtGui.QFrame.NoFrame)
        self.label_2.setObjectName(_fromUtf8("label_2"))
        self.gridLayout_3.addWidget(self.label_2, 1, 0, 1, 1)
        self.scrollArea = QtGui.QScrollArea(self.frame_2)
        self.scrollArea.setWidgetResizable(True)
        self.scrollArea.setObjectName(_fromUtf8("scrollArea"))
        self.scrollAreaWidgetContents = QtGui.QWidget()
        self.scrollAreaWidgetContents.setGeometry(QtCore.QRect(0, 0, 497, 229))
        self.scrollAreaWidgetContents.setObjectName(_fromUtf8("scrollAreaWidgetContents"))
        self.scrollArea.setWidget(self.scrollAreaWidgetContents)
        self.gridLayout_3.addWidget(self.scrollArea, 2, 0, 1, 1)
        spacerItem = QtGui.QSpacerItem(20, 40, QtGui.QSizePolicy.Minimum, QtGui.QSizePolicy.Minimum)
        self.gridLayout_3.addItem(spacerItem, 0, 0, 1, 1)
        self.gridLayout_4.addWidget(self.frame_2, 1, 0, 1, 1)
        self.frame_5 = QtGui.QFrame(Dialog)
        self.frame_5.setFrameShape(QtGui.QFrame.StyledPanel)
        self.frame_5.setFrameShadow(QtGui.QFrame.Raised)
        self.frame_5.setObjectName(_fromUtf8("frame_5"))
        self.gridLayout = QtGui.QGridLayout(self.frame_5)
        self.gridLayout.setObjectName(_fromUtf8("gridLayout"))
        self.select_file_txt = QtGui.QLineEdit(self.frame_5)
        self.select_file_txt.setStyleSheet(_fromUtf8("background-color: rgb(82, 82, 82);\n"
                                                     "color: rgb(255, 255, 255);"))
        self.select_file_txt.setLocale(QtCore.QLocale(QtCore.QLocale.English, QtCore.QLocale.UnitedStates))
        self.select_file_txt.setObjectName(_fromUtf8("select_file_txt"))
        self.gridLayout.addWidget(self.select_file_txt, 1, 0, 1, 1)
        self.select_file_btn = QtGui.QToolButton(self.frame_5)
        self.select_file_btn.setObjectName(_fromUtf8("select_file_btn"))
        self.gridLayout.addWidget(self.select_file_btn, 1, 1, 1, 1)
        self.label = QtGui.QLabel(self.frame_5)
        self.label.setObjectName(_fromUtf8("label"))
        self.gridLayout.addWidget(self.label, 0, 0, 1, 1)
        self.gridLayout_4.addWidget(self.frame_5, 3, 0, 2, 1)
        self.frame_3 = QtGui.QFrame(Dialog)
        self.frame_3.setFrameShape(QtGui.QFrame.StyledPanel)
        self.frame_3.setFrameShadow(QtGui.QFrame.Raised)
        self.frame_3.setObjectName(_fromUtf8("frame_3"))
        self.gridLayout_2 = QtGui.QGridLayout(self.frame_3)
        self.gridLayout_2.setObjectName(_fromUtf8("gridLayout_2"))
        self.format_lbl = QtGui.QLabel(self.frame_3)
        self.format_lbl.setObjectName(_fromUtf8("format_lbl"))
        self.gridLayout_2.addWidget(self.format_lbl, 1, 1, 1, 1)
        self.label_5 = QtGui.QLabel(self.frame_3)
        font = QtGui.QFont()
        font.setBold(True)
        font.setUnderline(True)
        font.setWeight(75)
        self.label_5.setFont(font)
        self.label_5.setObjectName(_fromUtf8("label_5"))
        self.gridLayout_2.addWidget(self.label_5, 0, 0, 1, 1)
        self.label_6 = QtGui.QLabel(self.frame_3)
        font = QtGui.QFont()
        font.setBold(True)
        font.setUnderline(True)
        font.setWeight(75)
        font.setKerning(False)
        self.label_6.setFont(font)
        self.label_6.setObjectName(_fromUtf8("label_6"))
        self.gridLayout_2.addWidget(self.label_6, 0, 1, 1, 1)
        self.data_lbl = QtGui.QLabel(self.frame_3)
        self.data_lbl.setObjectName(_fromUtf8("data_lbl"))
        self.gridLayout_2.addWidget(self.data_lbl, 1, 2, 1, 1)
        self.type_lbl = QtGui.QLabel(self.frame_3)
        self.type_lbl.setObjectName(_fromUtf8("type_lbl"))
        self.gridLayout_2.addWidget(self.type_lbl, 1, 0, 1, 1)
        self.data_title_lbl = QtGui.QLabel(self.frame_3)
        font = QtGui.QFont()
        font.setBold(True)
        font.setUnderline(True)
        font.setWeight(75)
        self.data_title_lbl.setFont(font)
        self.data_title_lbl.setObjectName(_fromUtf8("data_title_lbl"))
        self.gridLayout_2.addWidget(self.data_title_lbl, 0, 2, 1, 1)
        self.options_btn = QtGui.QToolButton(self.frame_3)
        self.options_btn.setObjectName(_fromUtf8("options_btn"))
        self.gridLayout_2.addWidget(self.options_btn, 1, 3, 1, 1)
        self.gridLayout_4.addWidget(self.frame_3, 2, 0, 1, 1)
        self.frame = QtGui.QFrame(Dialog)
        self.frame.setFrameShape(QtGui.QFrame.StyledPanel)
        self.frame.setFrameShadow(QtGui.QFrame.Raised)
        self.frame.setObjectName(_fromUtf8("frame"))
        self.horizontalLayout_3 = QtGui.QHBoxLayout(self.frame)
        self.horizontalLayout_3.setObjectName(_fromUtf8("horizontalLayout_3"))
        spacerItem1 = QtGui.QSpacerItem(40, 20, QtGui.QSizePolicy.Expanding, QtGui.QSizePolicy.Minimum)
        self.horizontalLayout_3.addItem(spacerItem1)
        self.select_all_btn = QtGui.QPushButton(self.frame)
        self.select_all_btn.setObjectName(_fromUtf8("select_all_btn"))
        self.horizontalLayout_3.addWidget(self.select_all_btn)
        self.deselect_all_btn = QtGui.QPushButton(self.frame)
        self.deselect_all_btn.setObjectName(_fromUtf8("deselect_all_btn"))
        self.horizontalLayout_3.addWidget(self.deselect_all_btn)
        self.invert_btn = QtGui.QPushButton(self.frame)
        self.invert_btn.setObjectName(_fromUtf8("invert_btn"))
        self.horizontalLayout_3.addWidget(self.invert_btn)
        spacerItem2 = QtGui.QSpacerItem(40, 20, QtGui.QSizePolicy.Fixed, QtGui.QSizePolicy.Minimum)
        self.horizontalLayout_3.addItem(spacerItem2)
        self.horizontalLayout_2 = QtGui.QHBoxLayout()
        self.horizontalLayout_2.setObjectName(_fromUtf8("horizontalLayout_2"))
        self.ok_btn = QtGui.QPushButton(self.frame)
        self.ok_btn.setStyleSheet(_fromUtf8("background-color: rgba(0, 128, 128, 154);"))
        self.ok_btn.setObjectName(_fromUtf8("ok_btn"))
        self.horizontalLayout_2.addWidget(self.ok_btn)
        self.cancel_btn = QtGui.QPushButton(self.frame)
        self.cancel_btn.setStyleSheet(_fromUtf8(""))
        self.cancel_btn.setObjectName(_fromUtf8("cancel_btn"))
        self.horizontalLayout_2.addWidget(self.cancel_btn)
        self.horizontalLayout_3.addLayout(self.horizontalLayout_2)
        self.gridLayout_4.addWidget(self.frame, 5, 0, 1, 1)

        self.retranslateUi(Dialog)
        QtCore.QMetaObject.connectSlotsByName(Dialog)

    def retranslateUi(self, Dialog):
        Dialog.setWindowTitle(_translate("Dialog", "Dialog", None))
        self.label_3.setText(_translate("Dialog", "Export Cleaned Lines", None))
        self.label_4.setText(_translate("Dialog", "Choose from validated molecules to be cleaned. Click \'OK\' to accept.", None))
        self.label_2.setText(
            _translate("Dialog", "Please check the validated molecules to be removed from remaining lines:", None))
        self.select_file_btn.setText(_translate("Dialog", "...", None))
        self.label.setText(_translate("Dialog", "Export cleaned lines to:", None))
        self.format_lbl.setText(_translate("Dialog", "Delimiter", None))
        self.label_5.setText(_translate("Dialog", "Type", None))
        self.label_6.setText(_translate("Dialog", "Format", None))
        self.data_lbl.setText(_translate("Dialog", "Space", None))
        self.type_lbl.setText(_translate("Dialog", "Text File (.txt)", None))
        self.data_title_lbl.setText(_translate("Dialog", "Delimiter", None))
        self.options_btn.setText(_translate("Dialog", "...", None))
        self.select_all_btn.setText(_translate("Dialog", "Select All", None))
        self.deselect_all_btn.setText(_translate("Dialog", "Deselect All", None))
        self.invert_btn.setText(_translate("Dialog", "Invert", None))
        self.ok_btn.setText(_translate("Dialog", "OK", None))
        self.cancel_btn.setText(_translate("Dialog", "Cancel", None))

